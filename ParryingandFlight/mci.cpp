...
void mci::PlayEffect(LPCWSTR _soundname)
{
	...
	if (mciSendCommand(NULL, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_ELEMENT, (DWORD_PTR)&openEffect)) {
		// Error handling
		return;
	}
	DWORD Effectid = openEffect.wDeviceID;

	// PLAY
	MCI_PLAY_PARMS playEffect;
	if (mciSendCommand(Effectid, MCI_PLAY, MCI_NOTIFY, (DWORD_PTR)&playEffect)) {
		// Error handling
		mciSendCommand(Effectid, MCI_CLOSE, 0, NULL);
		return;
	}
